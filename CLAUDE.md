...
- **After ANY `app/` change**: deploy mobile immediately. JS/assets ship to EAS channel `preview`; new native dependencies, permissions, or native config require bumping `expo.version` in `app/app.json`, then `cd app && python3 ../scripts/mobile_api_preflight.py eas build --platform ios --profile preview` and installing the replacement from the build link. Initial OTA-enabled build uses version 1.0.0.
...
- **Private mobile API**: `app/app.config.js` reads `PETRARCA_API_URL`; the publish script derives it from `~/.config/petrarca/mobile-api-capability` (0600) and verifies HTTPS health plus resolved Expo config before publishing. Never publish a bare `eas update` without this configuration. This is a single-user capability, as in Alif: possession of the installed app grants access. Never commit/print it, embed it in public web builds, or reuse the narrower Companion capability.
- **Gateway deployment**: after commit + push, `bash scripts/deploy-mobile-api.sh` installs the explicit mobile route allowlist through nginx with rollback on validation/reload failure. Server secret: `/etc/petrarca-mobile-capability` (root:root 0600). Keep `8090` loopback-only. For new native builds, use `python3 ../scripts/mobile_api_preflight.py eas build --platform ios --profile preview` from `app/` so the embedded release has the same API configuration.

...
import { Highlight, InterestTopic } from '../data/types';
        ...
        switch (block.type) {
          ...
          case 'ul':
            return (
              <Pressable
                key={i}
                style={[styles.markdownList, isHighlighted && styles.paragraphHighlight]}
...
export default function ReaderScreen() {
  ...
  // Scroll handler — save position periodically
  ...
  const fullContent = article.sections && article.sections.length > 0
    ? article.sections
          ...
          <View style={styles.noveltyCard}>
            ...
            {article.novelty_claims.map((nc, i) => (
              <View key={i} style={styles.noveltyItem}>
                <Text style={styles.noveltyBullet}>{'·'}</Text>

...
cd app && python3 ../scripts/mobile_api_preflight.py eas build --platform ios --profile preview
...
### Native startup and private API (September 8, 2026)
The first standalone release retained the obsolete public `:8090` default even
though the August containment bound the API to loopback. This caused native
requests to time out. Awaited book sync and first-launch article downloads kept
the splash visible until those requests finished. Initialization now waits only
for local state, reconciles books in the background, and notifies mounted library
views when the server response arrives. Content sync also runs in the background.

Mobile uses a separate 256-bit HTTPS capability and an explicit method/path
allowlist generated by `scripts/install_mobile_api.py`. It does not grant admin,
bulk ingestion, database, or arbitrary filesystem routes. The existing Companion
capability and public content allowlist retain their scopes. Request/error logs
are disabled in the mobile namespace to avoid recording its capability.

`app/deploy-mobile.sh` requires a 0600 capability file at
`~/.config/petrarca/mobile-api-capability`; its preflight checks the live HTTPS API
and confirms Expo config includes the correct URL before publication. Never put
the actual URL in Git or output. The native app is intentionally single-user;
its bundle/manifest contains the capability, matching Alif's deployment model.
Use `bash scripts/deploy-mobile-api.sh` for gateway changes. New native builds
must also be run through `scripts/mobile_api_preflight.py`.

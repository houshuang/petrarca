## 2026-09-22 — Unified batch released and genuine backlog verified

The pre-registered unified operator is deployed at `108bad1`, using current Limbic orchestration, source evidence, reusable results and independent review while Petrarca remains canonical. Four genuine outstanding wonderings produced four readings and six unselected quiz targets. A factual error held the first Italy draft; a narrower source-bound revision passed review. A deliberate interruption resumed cleanly; final retry performed fresh discovery, zero author/reviewer calls and zero publication. Fifty-four affected tests passed. Independent export verification preserved fifteen tables byte-for-byte, including 99 items, 106 positions, 15 runs and 394 events. Seven active readings now exist. The participant has not tried the pilot, so utility remains unobserved. See [deployment, content and recovery evidence](norway-reading-study/unified-batch.md); earlier entries below remain unchanged.

## 2026-09-22 — Unified resumable Norway source and reading batch (registered before changes)

Build one bounded operator invocation that discovers tagged original recordings, reports already processed material, archives and registers new originals, drafts source-anchored practice questions and only explicit one-step wonderings, checks semantic overlap against canonical content and the current batch, obtains independent review, and publishes accepted content through existing canonical SQLite actions. A read-only current snapshot and hashes must bind review and publication; failed, omitted, unavailable and uncertain work stays visible. Resume must reuse model outputs and reconcile lost publication receipts without duplicate writes. Keep the phone API and historical runs, positions, schedules and observations unchanged. Use GPT-5.6 Sol for authoring and GPT-6 Astra for review, with a strict one-worker, bounded-call budget and no recurring collector. No utility inference from the three pilot readings before the reader tries them.

Before any real publication, verify discovery completeness, original audio checksums, exact transcript spans, current corpus fingerprint and review coverage. Test recovery and fail-closed paths in isolated SQLite. This entry registers intended implementation, not an outcome.

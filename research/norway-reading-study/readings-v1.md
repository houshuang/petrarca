...
Released code: `4f5b009ac737c62c9885503f692fa06e069fd3ac` (server and iOS preview/runtime 1.0.0). EAS group: `ece28250-1624-4d49-8ab9-51713e45e90a`; iOS update: `01a0c976-dd12-77ba-a760-a09d447e22a3`. [Published update](https://expo.dev/accounts/houshuang/projects/petrarca/updates/ece28250-1624-4d49-8ab9-51713e45e90a). Open Petrarca briefly to download, then quit and reopen to apply. Physical-device adoption is not yet observed.

The unified deployment passed TypeScript, server update and service health. Its npm install removed 25 `peer: true` lockfile flags, triggering the gateway's tracked-clean guard. The exact generated diff was archived privately and checked to contain only those 25 removals before restoring that attributable file; gateway installation and the standard mobile deploy then passed. Unrelated server runtime files were preserved.

Private canonical import returned the three expected brief IDs; an identical retry returned `added=[]` and `duplicate=true`. The authenticated mobile route returned three readings, each with two unselected targets. Empty selection was rejected with HTTP 400. No production selection or synthetic learner event was used for verification.

The post-release export's hashes were independently verified. All 14 prior exported tables have byte-identical hashes and row counts: 99 items, 106 positions, 10 sources, 8 revisions, 14 frozen runs and 355 learner events remain unchanged. The only new rows are 3 reading intents, 3 briefs, 6 target definitions and 6 brief/target links; selections remain zero. The private post-export and machine-readable preservation receipt live beside the pre-export. This is a content/UX intervention, not a new learning outcome. The final browser replay also verified the learning-aids entry returns directly to the same practice card.

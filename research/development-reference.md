...
**Total**: 10,589 lines added, 617 deleted across 24 files.

**Deployed**: `exp://alifstian.duckdns.org:8082`
...
Readwise Reader ────┘                    └─→ concepts.json ─→ app bundle
...
- `Article`: id, title, author, source_url, hostname, date, content_markdown, sections[], one_line_summary, full_summary, key_claims[], topics[], estimated_read_minutes, content_type, word_count, sources[]
...
**Current output**: 51 articles, 69 concepts
...
| File | Lines | Purpose |
|------|-------|---------|
| `data/types.ts` | 115 | All TypeScript interfaces and type aliases |
| `data/store.ts` | 630 | Central state management — articles, concepts, reading states, signals, reviews, novelty scoring, scheduling, transcript→concept matching |
| `data/transcription.ts` | 130 | Soniox voice note transcription — upload, poll, retrieve, concept matching |
| `data/persistence.ts` | 110 | AsyncStorage load/save pairs for all state |
| `data/logger.ts` | 103 | JSONL interaction logging with sequential write queue |
| `data/articles.json` | — | 51 articles (bundled, generated by pipeline) |
| `data/concepts.json` | — | 69 concepts (bundled, generated by pipeline) |
| `app/_layout.tsx` | 86 | Tab navigator: Feed, Library, Review, Progress + hidden Reader |
| `app/reader.tsx` | 992 | Fluid depth reader: scrollable document, claim signals, voice recording, implicit tracking |
| `app/index.tsx` | 969 | Feed screen: List/Topics/Triage modes, novelty badges, continue reading |
| `app/review.tsx` | 443 | Spaced attention review: prompt→respond→rate flow, voice transcript display |
| `app/stats.tsx` | 382 | Progress: reading stats, knowledge map, voice notes with concept pills, event log |
| `app/library.tsx` | — | Previously-read articles |

### Pipeline (`scripts/`)

| File | Lines | Purpose |
|------|-------|---------|
| `build_articles.py` | 993 | Full pipeline: sources → fetch → LLM process → concepts |
| `deploy.sh` | 19 | Rsync to Hetzner + start Expo server |

### Data (`data/`)

| File | Purpose |
|------|---------|
| `articles.json` | 51 processed articles with sections, summaries, claims |
| `concepts.json` | 69 extracted concepts across 39 topics |
...
2. **Background research agents**: User reads an article, taps "research this" → spawns a background agent on Hetzner VM that finds diverse perspectives and related articles. Results waiting next time user opens app. This was a key interview request.

3. **Cross-article synthesis**: When multiple articles cover the same topic, generate a synthesis view ("Here's what 5 articles say about X, where they agree, where they disagree").

4. **Search**: No search across articles or concepts. Even basic text search would be valuable.

5. **Cluster summaries**: Topic clustering (2B) was built but without the "read the cluster summary" LLM-generated synthesis feature.
...
9. **Progressive summarization** (Tiago Forte method): Multi-pass highlighting where user's highlights become the summary for next review.

### Infrastructure

10. **Backend server**: Everything is currently client-side. A backend would enable: sync across devices, background processing, research agents, transcription jobs.

11. **More content**: 51 articles is good for testing but the Readwise source has 11,598 items. Could easily have hundreds or thousands.
...
### Why static JSON bundles instead of a database?
Articles and concepts are generated offline by the Python pipeline and bundled as JSON. This means content updates require rebuilding and redeploying. For a research prototype testing UI experiments, this is the right trade: no backend complexity, instant load, works offline. Would need to change for production.

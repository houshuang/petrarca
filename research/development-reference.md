...
| File | Lines | Purpose |
|------|-------|---------|
| `data/types.ts` | ~130 | All TypeScript interfaces — Article, Concept, Highlight, TopicSynthesis, etc. |
| `data/store.ts` | ~670 | Central state management — articles, concepts, highlights, reading states, signals, reviews, novelty scoring, scheduling, transcript→concept matching |
| `data/content-sync.ts` | ~122 | Remote content sync — manifest check, download articles/concepts/syntheses, local cache |
...
| `app/reader.tsx` | ~1400 | Fluid depth reader: highlighting, claim signals, dedup banner, research badge, voice recording |
